...
function prepareMessage(submission, orgMessage, poolData) {
  return new EmbedBuilder()
    ...
    .setAuthor({
      name: `Artist: ${submission.username}`,
      url: `https://inkbunny.net/${submission.username}`,
    })
    .addFields([
      {
        name: 'Type',
        value: `${submission.type_name}`,
        inline: true,
      },
      {
        name: 'Rating',
        value: `:regional_indicator_${submission.rating_name.toLowerCase()
          .slice(0, 1)}:`,
        inline: true,
      },
      {
      ...
      {
        name: 'Full picture link',
        value: submission.file_url_full,
      },
    ])
    .setImage(submission.file_url_full)
    .setFooter({
      text: 'Picture from inkbunny.net',
      iconURL: IBconfig.logo,
    });
}
...
module.exports.run = async (interaction, orgMessage, submission) => {
  ...
  const message = await orgMessage.edit({
    embeds: [embed],
    components: [buttons],
    fetchReply: true,
  });
